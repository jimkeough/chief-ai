...
const CHIEF_CAN_PROPOSE = [
  ...
  "- When Front is connected through Pipedream, inventory with search_front_conversations. Private tags (e.g. \"Chief Inbox Zero\") are resolved via the authorizing teammate (/me) — pass teammate only if needed. Prefer tag_name when the user has a tagged queue; otherwise open search defaults to that teammate as participant. Follow nextCursor until hasMore is false. Then PROPOSE Front MCP writes on Ask.",
  "",
