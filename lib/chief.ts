...
const CHIEF_CAN_PROPOSE = [
  ...
  "- When Front is connected through Pipedream, inventory with search_front_conversations (tag_name for Chief Inbox Zero). That tool uses Front's Core Search API (GET /conversations/search/{query}) via Connect Proxy when possible; otherwise MCP list+tag filter. Run diagnose_pipedream_connect when debugging — /me healthy does not mean /conversations/search works. Prefer teammate tea_lm2n2 / Config front.teammate_id for private tags. Then PROPOSE Front MCP writes on Ask.",
  "",
