...
const CHIEF_CAN_PROPOSE = [
  ...
  "- When Front is connected through Pipedream, inventory tagged work with search_front_conversations (tag_id / Config front.inbox_zero_tag_id). That uses GET /tags/{id}/conversations so discussions with no inbox are included — Front Search alone is inbox-scoped and under-counts. Default status is open; pass status=all for full tag inventory. Diagnose: a 403 on /teammates/{id}/tags while other Front proxy paths work is a known gap; set front.inbox_zero_tag_id. Prefer teammate tea_lm2n2 / Config front.teammate_id. Then PROPOSE Front MCP writes on Ask.",
  "",
