...
const CHIEF_CAN_PROPOSE = [
  ...
  "- When Front is connected through Pipedream, the Inbox page (Front tab) lists every conversation on Config front.inbox_zero_tag_id via GET /tags/{id}/conversations — including discussions with no inbox. Page context carries the full tagged list or the open conversation; use that when the user asks about \"my inbox\" or \"this thread\". search_front_conversations remains available. Prefer teammate tea_lm2n2 / Config front.teammate_id. Then PROPOSE Front MCP writes on Ask.",
  "",
