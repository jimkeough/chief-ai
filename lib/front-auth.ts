...
function parseConfigInput(raw: unknown): {
  ...
  return {
    clientId,
    clientSecret,
    scopes: normalizeFrontScopes(body.scopes),
  };
}
...
async function discoverFrontOAuth(): Promise<{
  ...
  return {
    ...
    resource: resourceUrlFromServerUrl(FRONT_MCP_URL),
  };
}
...
export async function saveFrontOAuthConfig(
  ...
  const admin = createAdminClient();
  const { error } = await admin.rpc("chief_front_upsert_config", {
    p_user_id: userId,
    p_client_id: input.clientId,
    p_client_secret: input.clientSecret,
    p_scopes: input.scopes,
  });
  if (error) throw new Error(`Could not save Front OAuth setup: ${error.message}`);
  return {
    configured: true,
    connected: false,
...
export function publicFrontOAuthError(
  ...
  if (
    /client|scope|authorization|oauth|front|credential|redirect|grant|connect/i.test(
      message,
    )
  ) {
